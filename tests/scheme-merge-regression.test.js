...
function setFieldValue(scheme, fieldName, value) {
  switch (fieldName) {
    case 'remark': return { ...scheme, remark: value };
...
assert(conflictItem.fieldDiffs.length === ALL_FIELD_NAMES.length, `${ALL_FIELD_NAMES.length}个字段差异`);
...
assert(mergeLogA.fieldSources.length === ALL_FIELD_NAMES.length, `merge日志有${ALL_FIELD_NAMES.length}个fieldSources`);
...
assert(mergeLog6.fieldSources.length === ALL_FIELD_NAMES.length, '重启后合并日志fieldSources完整');
...
console.log('\n【测试12】Remark 字段合并验证 - Remark field merge verification');
let state12 = createInitialState();
const { state: s12, scheme: scheme12 } = createImportScheme(state12, '备注方案', {
  remark: '这是原始备注信息',
  defaultBatch: { batchNoPattern: 'ORIG-{DATE}', batchNamePattern: '原始批次' },
  validationToggles: { ...defaultValidationToggles },
});
state12 = s12;

assert(scheme12.remark === '这是原始备注信息', '原始方案remark字段存在且正确');

const remarkMergeJSON = JSON.stringify({
  version: 1,
  exportedAt: new Date().toISOString(),
  exportedBy: '外部用户',
  schemes: [{
    ...
    defaultBatch: { batchNoPattern: 'NEW-{DATE}', batchNamePattern: '原始批次' },
    validationToggles: { ...defaultValidationToggles },
    isShared: false,
    isLocked: false,
  }, {
    ...
    remark: '新方案的备注内容',
    columnMappings: [{ csvColumn: '编号', targetField: 'sampleNo' }],
    defaultBatch: { batchNoPattern: 'NEWSCHEME-{DATE}', batchNamePattern: '新方案批次' },
    validationToggles: { ...defaultValidationToggles },
    isShared: false,
    isLocked: false,
  }],
...
const preview12 = previewSchemeMerge(state12, remarkMergeJSON);
assert(preview12.totalIncoming === 2, '预览：共2个方案');
assert(preview12.conflictCount === 1, '预览：1个冲突');
assert(preview12.newCount === 1, '预览：1个新方案');

const conflict12 = preview12.conflictItems.find((c) => c.existingScheme.name === '备注方案');
assert(conflict12 !== undefined, '冲突项：备注方案存在');

const remarkDiff12 = conflict12.fieldDiffs.find((d) => d.fieldName === 'remark');
assert(remarkDiff12 !== undefined, 'remark字段在fieldDiffs中存在');
assert(remarkDiff12.fieldLabel === '备注', 'remark字段label为"备注"');
assert(remarkDiff12.isSame === false, 'remark字段isSame为false（值不同）');
assert(remarkDiff12.originalValue === '这是原始备注信息', 'remark字段originalValue正确');
assert(remarkDiff12.newValue === '这是更新后的备注信息', 'remark字段newValue正确');
assert(remarkDiff12.originalDisplay === '这是原始备注信息', 'remark字段originalDisplay正确');
assert(remarkDiff12.newDisplay === '这是更新后的备注信息', 'remark字段newDisplay正确');

const batchDiff12 = conflict12.fieldDiffs.find((d) => d.fieldName === 'defaultBatch.batchNamePattern');
assert(batchDiff12 !== undefined, 'batchNamePattern字段在fieldDiffs中存在');
assert(batchDiff12.isSame === true, 'batchNamePattern字段isSame为true（值相同）');

const resolutions12 = {};
resolutions12[scheme12.id] = {};
for (const diff of conflict12.fieldDiffs) {
  if (diff.fieldName === 'remark') {
    resolutions12[scheme12.id][diff.fieldName] = 'use_new';
  } else if (diff.fieldName === 'defaultBatch.batchNoPattern') {
    resolutions12[scheme12.id][diff.fieldName] = 'use_new';
  } else {
    resolutions12[scheme12.id][diff.fieldName] = diff.isSame ? 'keep_original' : 'use_new';
  }
}

const merge12 = mergeImportSchemes(state12, preview12, resolutions12);
assert(merge12.success === true, '合并成功');
assert(merge12.mergedCount === 1, '合并1个冲突');
assert(merge12.newCount === 1, '新增1个方案');
state12 = merge12.state;

const merged12 = state12.importSchemes.find((s) => s.name === '备注方案');
assert(merged12.remark === '这是更新后的备注信息', '合并后remark字段采用新值');
assert(merged12.defaultBatch.batchNoPattern === 'NEW-{DATE}', '合并后batchNoPattern采用新值');
assert(merged12.defaultBatch.batchNamePattern === '原始批次', '合并后batchNamePattern保持原值');

const newScheme12 = state12.importSchemes.find((s) => s.name === '带备注的新方案');
assert(newScheme12 !== undefined, '新方案存在');
assert(newScheme12.remark === '新方案的备注内容', '新增方案remark字段正确');

const mergeLogRemark12 = state12.schemeMergeLogs.find(
  (l) => l.action === 'merge' && l.schemeName === '备注方案'
);
assert(mergeLogRemark12 !== undefined, '存在备注方案的合并日志');
const remarkSource12 = mergeLogRemark12.fieldSources.find((f) => f.fieldName === 'remark');
assert(remarkSource12 !== undefined, '合并日志fieldSources包含remark');
assert(remarkSource12.source === 'new', 'remark字段source为new');

const undo12 = undoLastSchemeMerge(state12);
assert(undo12.success === true, '撤销成功');
state12 = undo12.state;

const restored12 = state12.importSchemes.find((s) => s.name === '备注方案');
assert(restored12.remark === '这是原始备注信息', '撤销后remark恢复为原始值');
assert(restored12.defaultBatch.batchNoPattern === 'ORIG-{DATE}', '撤销后batchNoPattern恢复原始值');
assert(!state12.importSchemes.some((s) => s.name === '带备注的新方案'), '撤销后新方案被移除');

console.log('\n【测试13】导出再导回一致性 - Export-import roundtrip consistency');
let state13 = createInitialState();
const { state: s13a, scheme: origScheme13 } = createImportScheme(state13, '往返方案', {
  remark: '往返测试的备注内容',
  defaultBatch: { batchNoPattern: 'RT-{DATE}', batchNamePattern: '往返批次' },
  validationToggles: {
    skipEmptySampleNo: false,
    skipDuplicateInFile: true,
    skipDuplicateInBatch: false,
    skipInvalidQuantity: true,
    skipEmptySource: false,
  },
  columnMappings: [
    { csvColumn: 'A列', targetField: 'sampleNo' },
    { csvColumn: 'B列', targetField: 'quantity' },
    { csvColumn: 'C列', targetField: 'source' },
  ],
});
state13 = s13a;
...
const exportData13 = {
  ...
  exportedAt: new Date().toISOString(),
  exportedBy: '操作员小王',
  schemes: [origScheme13],
};
const exportJSON13 = JSON.stringify(exportData13, null, 2);

let state13b = createInitialState();
const preview13 = previewSchemeMerge(state13b, exportJSON13);
assert(preview13.totalIncoming === 1, '导入预览：1个方案');
assert(preview13.newCount === 1, '导入预览：1个新方案');
assert(preview13.conflictCount === 0, '导入预览：0个冲突');

const resolutions13 = {};
const merge13 = mergeImportSchemes(state13b, preview13, resolutions13);
assert(merge13.success === true, '导入合并成功');
state13b = merge13.state;

const imported13 = state13b.importSchemes.find((s) => s.name === '往返方案');
assert(imported13 !== undefined, '导回后方案存在');
assert(imported13.remark === '往返测试的备注内容', '导回后remark一致');
assert(imported13.defaultBatch.batchNoPattern === 'RT-{DATE}', '导回后batchNoPattern一致');
assert(imported13.defaultBatch.batchNamePattern === '往返批次', '导回后batchNamePattern一致');
assert(imported13.validationToggles.skipEmptySampleNo === false, '导回后skipEmptySampleNo一致');
assert(imported13.validationToggles.skipDuplicateInBatch === false, '导回后skipDuplicateInBatch一致');
assert(imported13.validationToggles.skipEmptySource === false, '导回后skipEmptySource一致');
assert(imported13.columnMappings.length === 3, '导回后columnMappings数量一致');
assert(imported13.columnMappings[0].csvColumn === 'A列', '导回后columnMappings[0].csvColumn一致');
assert(imported13.columnMappings[1].targetField === 'quantity', '导回后columnMappings[1].targetField一致');
...
const exportJSON13b = JSON.stringify(exportData13b, null, 2);

let state13c = createInitialState();
const { state: s13c, scheme: existingScheme13 } = createImportScheme(state13c, '往返方案', {
  remark: '旧备注',
  defaultBatch: { batchNoPattern: 'OLD', batchNamePattern: '旧批次' },
});
state13c = s13c;

const preview13c = previewSchemeMerge(state13c, exportJSON13b);
assert(preview13c.conflictCount === 1, '再次导入：1个冲突');

const conflict13c = preview13c.conflictItems[0];
const resolutions13c = {};
resolutions13c[existingScheme13.id] = {};
for (const diff of conflict13c.fieldDiffs) {
  resolutions13c[existingScheme13.id][diff.fieldName] = diff.isSame ? 'keep_original' : 'use_new';
}

const merge13c = mergeImportSchemes(state13c, preview13c, resolutions13c);
assert(merge13c.success === true, '再次合并成功');
state13c = merge13c.state;

const reImported13 = state13c.importSchemes.find((s) => s.name === '往返方案');
assert(reImported13.remark === '往返测试的备注内容', '合并remark字段正确');
assert(reImported13.defaultBatch.batchNoPattern === 'RT-{DATE}', '合并batchNoPattern字段正确');
assert(reImported13.columnMappings[2].csvColumn === 'C列', '合并columnMappings正确');
assert(state13c.importSchemes.filter((s) => s.name === '往返方案').length === 1, '没有重复的方案');
